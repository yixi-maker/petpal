...
| lat | Float | 纬度 |
| lng | Float | 经度 |
| geohash | String? | geohash 编码（用于附近查询） |
| fuzzyDistance | String? | 模糊距离展示文本，如"800m 内" |
| updatedAt | DateTime | |

前端仅展示 `fuzzyDistance` 或从 `city` / `district` 派生的模糊文本（如"同城区附近""约 2.3km"），绝不渲染 `lat`/`lng` 原始值。
...
| actionType | Enum(HIDE_POST, HIDE_COMMENT, BAN_USER, UNBAN_USER, RESOLVE_REPORT, DISMISS_REPORT, HIDE_PLACE, HIDE_REVIEW) | |
| targetType | Enum(POST, COMMENT, USER, REPORT, PLACE, PLACEREVIEW, PET) | |
| targetId | Int | |
| detail | String? | 操作说明 |
| createdAt | DateTime | |
...
- 默认按 geohash 前缀匹配排序，再按 fuzzyDistance 升序
- 前端绝不渲染 lat/lng 原始值
- API 返回时不包含 lat/lng 字段
...
- 所有管理操作记录到 AdminAction 表
- 封禁用户 → status=BANNED，该用户无法登录
- 隐藏内容 → status=HIDDEN，前端查询时排除
